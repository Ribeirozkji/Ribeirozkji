# Pedro Henrique

Desenvolvedor Full-Stack em formação | React, Node.js, JavaScript e MySQL | Construindo aplicações web, APIs e projetos com foco em organização, usabilidade e boas práticas.

Este perfil reúne projetos, estudos e evolução técnica voltados à construção de aplicações web, APIs, bancos de dados e boas práticas de desenvolvimento. Também estudo fundamentos de cibersegurança e infraestrutura para ampliar minha visão sobre software, segurança e ambientes de aplicação.

## Objetivo profissional

Busco oportunidades de estágio ou posição júnior em desenvolvimento web/backend, onde eu possa aplicar meus conhecimentos em projetos reais, evoluir tecnicamente e contribuir com soluções bem estruturadas.

## Contato

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/pedro-henrique-14bba7303)
[![GitHub](https://img.shields.io/badge/GitHub-111111?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Ribeirozkji)

## Stack

### Linguagens
![JavaScript](https://img.shields.io/badge/JavaScript-111111?style=for-the-badge&logo=javascript&logoColor=F7DF1E)

### Front-end
![HTML5](https://img.shields.io/badge/HTML5-111111?style=for-the-badge&logo=html5&logoColor=E34F26)
![CSS3](https://img.shields.io/badge/CSS3-111111?style=for-the-badge&logo=css3&logoColor=1572B6)
![React](https://img.shields.io/badge/React-111111?style=for-the-badge&logo=react&logoColor=61DAFB)
![Vite](https://img.shields.io/badge/Vite-111111?style=for-the-badge&logo=vite&logoColor=646CFF)

### Back-end
![Node.js](https://img.shields.io/badge/Node.js-111111?style=for-the-badge&logo=node.js&logoColor=5FA04E)

### Banco de Dados
![MySQL](https://img.shields.io/badge/MySQL-111111?style=for-the-badge&logo=mysql&logoColor=4479A1)

### Versionamento
![Git](https://img.shields.io/badge/Git-111111?style=for-the-badge&logo=git&logoColor=F05032)
![GitHub](https://img.shields.io/badge/GitHub-111111?style=for-the-badge&logo=github&logoColor=white)

## Atualmente estudando

- Desenvolvimento de APIs REST com Node.js
- Integração entre front-end, back-end e banco de dados
- Boas práticas com Git e GitHub
- Fundamentos de cibersegurança
- Estruturação de projetos web

## Projetos principais

### ProjectFlow
Aplicação web de gestão de projetos com linha do tempo interativa, modo foco gamificado e dashboard de produtividade. O projeto foi desenvolvido para praticar organização de tarefas, controle visual de progresso e construção de interfaces funcionais.

**Tecnologias:** React, JavaScript, HTML, CSS e Git  
**Principais aprendizados:** componentização, organização de interface, manipulação de estado e estruturação de funcionalidades.

### AquaLimpaOFC
Projeto web voltado à conscientização sobre preservação da água e sustentabilidade. A aplicação apresenta informações, recursos visuais e estrutura de página para comunicar boas práticas ambientais de forma acessível.

**Tecnologias:** HTML, CSS e JavaScript  
**Principais aprendizados:** estruturação semântica, responsividade, organização visual e comunicação de conteúdo.

### ConectaTudo
Aplicação desenvolvida em TypeScript com foco em organização de funcionalidades e construção de uma interface mais estruturada. O projeto foi criado para praticar tipagem, organização de código e evolução de aplicações web.

**Tecnologias:** TypeScript, JavaScript, HTML, CSS e Git  
**Principais aprendizados:** tipagem, estruturação de projeto, organização de componentes e manutenção de código.

## Estatísticas

<p align="center">
  <img height="170" src="https://github-readme-stats.vercel.app/api?username=Ribeirozkji&show_icons=true&theme=github_dark&hide_border=true&cache_seconds=1800" />
  <img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Ribeirozkji&layout=compact&theme=github_dark&hide_border=true&cache_seconds=1800" />
</p>

<p align="center">
  <img height="170" src="https://streak-stats.demolab.com?user=Ribeirozkji&theme=github-dark&hide_border=true" />
</p>
